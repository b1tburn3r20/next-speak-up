import { Button } from "@/components/ui/button";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";

export type ShareDataObject = {
  title: string;
  text: string;
  url: string;
};

interface PageShareButtonProps {
  shareData: ShareDataObject;
}

const PageShareButton = ({ shareData }: PageShareButtonProps) => {
  const handleShareClick = async () => {
    await navigator.share(shareData);
  };

  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger>
          <Button
            onClick={handleShareClick}
            variant="ghost"
            className="text-muted-foreground cursor-pointer p-1"
            size="icon"
            asChild
          >
            <svg
              fill="hsl(var(--muted-foreground))"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 512 512"
            >
              <path d="M503.7 189.8L327.7 37.9C312.3 24.5 288 35.3 288 56v80.1C127.4 137.9 0 170.1 0 322.3c0 61.4 39.6 122.3 83.3 154.1 13.7 9.9 33.1-2.5 28.1-18.6C66.1 312.8 132.9 274.3 288 272.1V360c0 20.7 24.3 31.5 39.7 18.2l176-152c11.1-9.6 11.1-26.8 0-36.3z" />
            </svg>
          </Button>
        </TooltipTrigger>
        <TooltipContent className="cursor-pointer" onClick={handleShareClick}>
          <p className="p-2 text-lg font-bold">Share</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
};

export default PageShareButton;
