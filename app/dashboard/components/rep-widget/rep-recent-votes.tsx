...
const POSITION_BADGE: Record<string, { label: string; variant: badgeVariantType }> = {
            ...
            <TableBody>
              {votes.length === 0 ? (
                ...
                votes.map((vote, i) => {
                  ...
                  const config = PolicyAreas[vote.policyArea as keyof typeof PolicyAreas]
                  ...
                  return (
                    <TableRow key={vote.memberVoteId} className={i % 2 === 0 ? "bg-muted/30" : ""}>
                      ...
                      <TableCell className="py-1.5 min-w-0">
                        <div className="text-xs font-medium line-clamp-2 wrap-word-break md:max-w-64">{title}</div>
                        {vote.billNumber && (
